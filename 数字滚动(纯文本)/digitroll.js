...
DigitRoll.prototype={
    ...
    roll:function (n) {
        ...
        Array.prototype.forEach.call(this.container.querySelectorAll('.num'), function (item,i) {
            ...
            // 做兼容性: 最低支持IE9
            function jrx(duration, translateY) {
                var arr = ['', '-webkit-', '-o-', '-ms-' , '-moz-'];
                var tdArr = [];
                var tArr = [];
                for (var i = 0, len = arr.length; i < len; i++) {
                    var td = arr[i] + 'transition-duration: ' + duration + ';';
                    var t = arr[i] + 'transform: translateY(-' + translateY + ');';
                    tdArr.push(td);
                    tArr.push(t);
                }
                return tdArr.join('') + tArr.join('');
            }
            // item.style.cssText += '-webkit-transition-duration:0s;-webkit-transform:translateY(0);';//重置位置
            item.style.cssText += jrx('0s', '0px');//重置位置
            ...
            setTimeout(function () {
               // item.style.cssText+='-webkit-transition-duration:1s;-webkit-transform:translateY(-'+self.rollHeight*gapNum+'px)';
                item.style.cssText+=jrx('1s', self.rollHeight * gapNum + 'px');
            },50)
        })
    },
    /** 
