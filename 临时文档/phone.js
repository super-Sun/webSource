 ...
 * - quality       {String}              图片输出质量，仅对 jpeg 格式的图片有效，取值 0 - 1，默认为0.8。（这个质量并不是图片的最终质量，而是在经过 lrz 插件压缩后的基础上输出的质量。相当于 quality * lrzOption.quality）
 ...
 * ----- quality         {Number}              图片压缩质量，仅对 jpeg 格式的图片有效，取值 0 - 1，默认为0.7。（这个质量不是最终输出的质量，与 options.quality 是相乘关系）
 *
...
(function(root, factory) {
    ...
    if (typeof module === 'object' && typeof exports === 'object') {
        ...
        root.takePhoto = factory(root.IScroll, root.Hammer, root.lrz);
    }
    ...
    function takePhoto(container, options) {
        if (!(this instanceof takePhoto)) {
            return new takePhoto(options);
        }
    ...
    var p = takePhoto.prototype;

    p.constructor = takePhoto;
    ...
    p._clipImg = function() {
        ...
        try {
            var dataURL = this._canvas.toDataURL(options.outputType, options.quality);
            if (this._viewList) {
                this._viewList.forEach(function($view, i) {
