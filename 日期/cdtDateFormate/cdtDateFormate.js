/**
 * Created by Sunluwei on 2017/9/29.
 */
...
(function (window, $, undefined) {
    ...
    /*************格式化方法开始**************/
        // 1.标准日期格式化
    var dateFormate = function (date, formate) {
            // 补0
            var zeroToNum = function (num) {
                if (num < 10) {
                    num = '0' + num;
                }
                return num;
            ...
            if (typeof date === 'string') {
                ...
                if (date.getMonth() < 12) {
                    ...
                    return '请输入合法的日期(error: 字符串转日期失败)';
                }
            }
            ...
            formate = formate.replace('yyyy', YYYY);
            formate = formate.replace('MM', zeroToNum(MM));
            formate = formate.replace('dd', zeroToNum(DD));
            formate = formate.replace('w', zeroToNum(week));
            formate = formate.replace('hh', zeroToNum(hh));
            formate = formate.replace('mm', zeroToNum(mm));
            formate = formate.replace('ss', zeroToNum(ss));
            formate = formate.replace('M', MM);
            formate = formate.replace('d', DD);
            formate = formate.replace('h', hh);
            formate = formate.replace('m', mm);
            formate = formate.replace('s', ss);
            return formate;
        };
    // 2.农历日期格式化
    var lunarFormate = function (date) {
        if (typeof date === 'string') {
            //将date字符串转化成日期
        ...
        var dataObj = ChineseCalendar.date2lunar(date);
        var dateStr = dataObj.gzY + '年' + dataObj.lunarMonthChiness + dataObj.lunarDayChiness;
        return dateStr;
    }
    var sampleDate = function (date, formate) {
        if (typeof date === 'string') {
            //将date字符串转化成日期
            date = new Date(date.replace(/-/g,"/"));
            if (date.getMonth() < 12) {
        ...
        var nowDD = nowdate.getDate(); // 获取当前日期
        var DD = date.getDate();
        var dz = ''
        var num = nowDD - DD;
        if (num < 0) {
        ...
        return dz + dateFormate(date, formate);
    }

    // 日期工具类
    var CDT_DateFormate = {
        formate: dateFormate,
        lunarFormate: lunarFormate,
        // TODO: 1.根据日期获取节日；2.根据日期获取; 3.获取星期...
        sampleDate: sampleDate
    }
    window.CDT_DateFormate = CDT_DateFormate;
})(window, jQuery);